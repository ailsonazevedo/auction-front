...
interface UploadPhotoFileProps {
...
const UploadPhotoFile = ({
  ...
  return (
    <Stack gap={2} justifyContent="center" textAlign="center" width={width}>
      <DropZone
        onClick={() => {
        ...
        style={{
          alignItems: "center",
          aspectRatio: isProfileImage ? "1/1" : "auto",
          borderColor:
            isError && formik.touched[name]
              ? theme.palette.error.main
              : theme.palette.divider,
          borderRadius: isProfileImage ? "50%" : theme.shape.borderRadius,
          display: "flex",
          flexDirection: "column",
          justifyContent: "center",
          width: isProfileImage ? "100%" : "auto",
        }}
        theme={theme}
      >
        {isUploading ? (
          <>
            <CircularProgress />
            <Typography variant={sizeStyle === "small" ? "caption" : "body2"}>
              Enviando...
            </Typography>
          </>
        ) : (
          <>
            {showPreview && file && formik.values[name] ? (
              <Box
                sx={{
                  "&:hover": {
                    "&::after": {
                  ...
                  backgroundImage: `url(${formik.values[name]})`,
                ...
                {file ? (
                  <Stack alignItems="center" spacing={1}>
                    <Tooltip title="Visualizar arquivo">
                      <Typography
                        onClick={() =>
                          window.open(formik.values[name], "_blank")
                        }
                        sx={{ cursor: "pointer", textDecoration: "underline" }}
                        variant={sizeStyle === "small" ? "caption" : "body2"}
                      >
                        {file.name.length > 30
                    ...
                    <Stack alignItems="center" direction="row" spacing={1}>
                      <Tooltip title="Remover arquivo">
                        <IconButton
                          disabled={isDisabled}
                          onClick={handleRevertUpload}
                          size={sizeStyle === "small" ? "small" : "medium"}
                        >
                          <DeleteIcon
                            fontSize={
                              sizeStyle === "small" ? "small" : "medium"
                            }
                          />
                        </IconButton>
                      </Tooltip>
